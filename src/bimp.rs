...
impl Amel {
    pub fn bimpunique(&self, pathfile: &str) -> Result<HashMap<String>, Box<dyn Error>> {
        let pathfileopen = File::open(pathfile).expect("file not found");
        let fileopen = BufReader::new(pathfileopen);
        let bimpvec: HashMap<String> = HashMap::new();
        for i in fileopen.lines() {
            ...
            if linevec[2] == "mRNA" {
                ...
                bimpvec.push(linesplit);
            }
        }
        Ok(amelvec)
    }

    pub fn bimpparse(&self, pathfile: &str) -> Result<Vec<Amel>, Box<dyn Error>> {
        ...
        for i in fileopen.lines() {
         ...
         let mut finalvec:Vec<Amel> = Vec::new();
         for i in mrna.iter(){
         for val in cds.iter(){
